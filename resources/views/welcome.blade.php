                ...
                <div class="h-full lg:grid grid-cols-1 gap-6">
                    @foreach ($products as $product)
                        <div style="background-image: url({{ asset('storage/' . $product->image) }})"
                            ...
                            <a href="#">
                                <h5 class="mb-2 text-2xl font-semibold tracking-tight text-gray-300">{{ $product->name }}
                                </h5>
                            </a>
            ...
            <div class="grid gap-8 mb-6 md:mb-16 md:grid-cols-2 md:grid-rows-2">
                ...
                @foreach ($popularCategories as $category)
                    <div class="col-span-1 p-6 bg-white border border-gray-200 rounded-lg shadow "">
                        <a href="#">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Category ::
                                {{ $category->name }}</h5>
                        </a>
        ...
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:px-6 ">
            ...
            <div class="grid gap-8 mb-6 md:mb-16 md:grid-cols-2 lg:grid-cols-4">
                ...
                @foreach ($newArrivals as $newArrival)
                    <div class="col-span-1 p-6 bg-white border border-gray-200 rounded-lg shadow "">
                        <a href="{{ route('product.show', $product->id) }}">
                            <img class=" rounded-lg" src="{{ asset('storage/' . $newArrival->image) }}" alt="product image" />
                        </a>

                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">{{ $newArrival->name }}</h5>
                        <p class="mb-3 font-normal text-gray-700 ">{{ $newArrival->description }}</p>
                        <a href="{{ route('product.show', $product->id) }}"
                            class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-primary rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">
                            Read more
                            <svg class="rtl:rotate-180 w-3.5 h-3.5 ms-2" aria-hidden="true"
